...
_RULE_OVERRIDES: dict[str, dict[str, object]] = {
    ...
    "credential-harvesting-exfiltration": {
        "title": "Systematic credential harvesting and exfiltration",
        "category": "credential-access",
        "evidence_class": "correlated",
        "default_severity": "HIGH",
        "description": "Detects multi-family credential collection, filesystem enumeration, serialization, and outbound writes in one source file across helper functions.",
    ...
    "credential-identifier-flow-to-network": {
        ...
        "evidence_class": "correlated",
        "default_severity": "HIGH",
        "description": "Tracks a credential-file read through identifier assignments into an explicit outbound request body.",
        "recommendation": "Block the extension and review the linked credential source, transformations, and destination.",
        "false_positive_notes": "Requires the same identifier lineage; unrelated credential and network tokens do not match.",
        "benchmark_tags": ["credential", "value-flow", "network"],
    },
    ...
    "remote-vsix-install-chain": {
        "title": "Unverified remote VSIX installation",
        "category": "execution",
        "evidence_class": "correlated",
        "default_severity": "HIGH",
